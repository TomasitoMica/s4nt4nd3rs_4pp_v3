...
echo '<root>';
	...
	echo '<sync_eventos>';
		$rs_eventos_del = mysql_query('SELECT eventos_id FROM eventos WHERE eventos_estado = -1 AND eventos_sync_value>"' . date('Y-m-d H:i:s', ($sync_value)) . '" ORDER BY eventos_id ASC LIMIT 100;');
		...
		echo '<data><![CDATA[';
		$rs_eventos = mysql_query('SELECT * FROM eventos WHERE eventos_fecha_hora>=NOW() AND eventos_estado = 1 AND eventos_sync_value>"' . date('Y-m-d H:i:s', ($sync_value)) . '" ORDER BY eventos_id ASC');
		$rows_eventos = array();
